    ...
    (note: number | string) => {
      ...
      if (note === 42 || note === "r") {
        startTransition(async () => {
          ...
          formData.append("lyric", `${tapRef.current.value}`);
          startTransition(async () => {
            if (tapRef.current.value !== "") {
              addOptimisticLyrics(tapRef.current.value);
          ...
          <div className="flex flex-col w-full items-center relative">
            <div className="flex">
              <input
                ...
                type="text"
                placeholder="🎨"
                className="text-center text-xs p-2 border-0 outline-hidden dark:bg-slate-800/10"
                name="style"
                autoComplete="on"
              />
              <input
