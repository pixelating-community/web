...
export async function editPerspective({
  ...
  try {
    ...
    const result =
      file && file.size > 0
        ? await handleFileUpload(file, data, perspectiveRow.objective_id)
        : await updatePerspectiveWithoutFile(data, id);

    await handleSampleUpdates(data, id);
...
const handleFileUpload = async (file: File, data: any, objectiveId: UUID) => {
...
const updatePerspectiveWithoutFile = async (data: any, id: UUID) => {
  return await sql`
...
const handleSampleUpdates = async (data: any, id: UUID) => {
  if (data.sample === "") {
