...
const getSampleBoundButtonClass = (status: SampleBoundSaveStatus) => {
  const statusClass = {
    error: "text-red-200",
...
type SWEFooterProps = {
  ...
  onSetSampleStart?: () => void;
  onSetSampleEnd?: () => void;
  sampleStartSaveStatus?: SampleBoundSaveStatus;
  sampleEndSaveStatus?: SampleBoundSaveStatus;
  sampleBoundsError?: string;
...
export const SWEFooter = ({
  ...
  onSetSampleStart,
  onSetSampleEnd,
  sampleStartSaveStatus = "idle",
  sampleEndSaveStatus = "idle",
  sampleBoundsError,
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          ...
          {!isViewer && !isMinimized && (
            <div className="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap sm:items-center">
              ...
              >
                {confirmClearAllMarks.armed ? "🗑️×1" : "🗑️×2 (all)"}
              </button>
            </div>
          )}
          {!isViewer &&
          ...
          {!isViewer &&
            !isMinimized &&
            (onSetSampleStart || onSetSampleEnd) && (
              <div className="flex items-center gap-2 rounded-xl bg-white/5 px-2 py-1 text-[10px] uppercase text-white/50 tracking-[0.2em]">
                {onSetSampleStart ? (
                  <button
                    type="button"
                    onClick={onSetSampleStart}
                    className={getSampleBoundButtonClass(sampleStartSaveStatus)}
                    disabled={
                      sampleStartSaveStatus === "saving" ||
                      sampleEndSaveStatus === "saving"
                    }
                    aria-label="Set sample start"
                    title="Set sample start from playback position"
                  >
                    {getSampleBoundButtonLabel({
                      base: "Start",
                      status: sampleStartSaveStatus,
                    })}
                  </button>
                ) : null}
                {onSetSampleEnd ? (
                  <button
                    type="button"
                    onClick={onSetSampleEnd}
                    className={getSampleBoundButtonClass(sampleEndSaveStatus)}
                    disabled={
                      sampleStartSaveStatus === "saving" ||
                      sampleEndSaveStatus === "saving"
                    }
                    aria-label="Set sample end"
                    title="Set sample end from playback position"
                  >
                    {getSampleBoundButtonLabel({
                      base: "End",
                      status: sampleEndSaveStatus,
                    })}
                  </button>
                ) : null}
                {sampleBoundsError ? (
              ...
              <div className="flex items-center gap-2">
                <div className="flex-1 min-w-0">
                  <Audio
                    ...
                    debugControls={isViewer && showViewerAudioControls}
        ...
        {!isViewer && !isMinimized ? children : null}
      </div>
    </div>
  );
};
