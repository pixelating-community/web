...
import {
  ...
  MIDI_DURATION_NUDGE_STEP_SECONDS,
  MIDI_FINE_NUDGE_CC,
  MIDI_FINE_NUDGE_STEP_SECONDS,
  MIDI_HIGHLIGHT_DURATION_CC,
  type MidiAccessLike,
  type MidiInputLike,
...
type UseSwMidiControlsArgs = {
  ...
  onShiftSelectedWordEnd: (delta: number) => void;
...
export const useSwMidiControls = ({
  ...
  const onShiftSelectedWordEndRef = useRef(onShiftSelectedWordEnd);
  ...
  onShiftSelectedWordEndRef.current = onShiftSelectedWordEnd;
  ...
  useEffect(() => {
    ...
    const handleMidiMessage = (event: MidiMessageEventLike) => {
      ...
      if (data1 === MIDI_FINE_NUDGE_CC) {
        onShiftSelectedWordStartRef.current(delta * MIDI_FINE_NUDGE_STEP_SECONDS);
        return;
      }

      if (data1 === MIDI_HIGHLIGHT_DURATION_CC) {
        onShiftSelectedWordEndRef.current(delta * MIDI_DURATION_NUDGE_STEP_SECONDS);
      }
    };
