...
export const buildPersistedAudioPatch = (audioSrc: string) => {
  const resolvedAudioSrc = audioSrc.trim();
  return {
...
export const useSwRecording = ({
  ...
  const setLocalAudioOverride = useCallback(
    (id: string, blob: Blob) => {
      const prev = runtimeById[id]?.localAudioOverride;
      if (prev) {
  ...
  const saveTimings = useCallback(
    async (
      ...
      const result = await savePerspectiveAlignmentFn({
        data: {
          actionToken,
          ...(payload.audioKey ? { audioKey: String(payload.audioKey) } : {}),
          ...(payload.audioUrl ? { audioUrl: String(payload.audioUrl) } : {}),
          ...(payload.clearAudio ? { clearAudio: true } : {}),
          ...(typeof payload.duration === "number"
            ? { duration: payload.duration }
            : {}),
          ...(typeof voiceOffsetSeconds === "number" &&
          voiceOffsetSeconds > 0
            ? { voiceOffsetSeconds }
            : {}),
          perspectiveId: id,
          timings,
      ...
      if (!result.ok) {
        if (result.code === "TOPIC_LOCKED" && topicName) {
  ...
  const handleRecorderStart = useCallback(
    ({ mimeType: _mimeType }: { mimeType: string }) => {
      ...
      recordingStartTimeRef.current = getAudioCurrentTime?.() ?? 0;
      setRecordingState(selectedPerspective.id, { status: "recording" });
    },
    [selectedPerspective, setRecordingState, getAudioCurrentTime],
  );
  ...
  const handleRecorderCapture = useCallback(
    async ({ blob, mimeType }: { blob: Blob; mimeType: string }) => {
      ...
      try {
        ...
        const filename = `studio-${perspectiveId}-${Date.now()}.${extension}`;
        ...
        const startOffset = recordingStartTimeRef.current;
        ...
        const resolvedRecordingSrc = result.recording_src?.trim() || upload.key;
        const resolvedAudioSrc = result.audio_src?.trim() || "";
        clearDraftTimings(perspectiveId);
        patchRuntime(perspectiveId, {
          recording: result.merging ? { status: "merging" } : { status: "idle" },
          timings: persistedTimings,
          lastSavedRevision: nextRevision,
          lastDraftRevision: nextRevision,
          dirtyTimings: false,
          localAudioOverride: undefined,
          recordingOverride: result.merging ? undefined : resolvedRecordingSrc,
          ...(resolvedAudioSrc
            ? buildPersistedAudioPatch(resolvedAudioSrc)
            : {}),
        });
        if (!result.merging) {
          ...
          es.onmessage = (event) => {
            try {
              ...
              if (data.status === "done") {
                ...
                patchRuntime(perspectiveId, {
                  recording: { status: "idle" },
                  recordingOverride: resolvedRecordingSrc,
                });
              }
            } catch {
              // ignore parse errors
