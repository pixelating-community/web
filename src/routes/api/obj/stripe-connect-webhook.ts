...
export const Route = createFileRoute("/api/obj/stripe-connect-webhook")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        try {
          ...
          console.warn("Rejected Stripe Connect webhook", {
            requestId,
        ...
        try {
          ...
          console.error("Failed to process Stripe Connect webhook", {
            requestId,
            eventId: event.id,
            error: error instanceof Error ? error.message : String(error),
          });
