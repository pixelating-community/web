...
import { deletePerspective } from "@/lib/deletePerspective.server";
...
const putSchema = z.object({
  ...
  hasImageSrc: z.boolean().optional(),
  ...
  server: {
    handlers: {
      PUT: async ({ request, params }) => {
        ...
        if (data.hasImageSrc) {
          formData.set("image_src", data.imageSrc?.trim() ?? "");
        }
      ...
      DELETE: async ({ request, params }) => {
        ...
        const topicRows = await sql`
          SELECT t.id AS topic_id, t.name AS topic_name, t.token AS topic_token, t.locked AS topic_locked
          FROM perspectives p
