...
export const encodeWav = (samples: Float32Array, sampleRate: number) =>
  encodeWavChannels([samples], sampleRate);

export const encodeDualMonoWav = (samples: Float32Array, sampleRate: number) =>
  encodeWavChannels([samples, samples], sampleRate);
...
export const hasSevereStereoImbalance = (
  buffer: AudioBuffer,
  ratioThreshold = 0.35,
) => {
  if (buffer.numberOfChannels < 2 || buffer.length === 0) {
    return false;
  }

  const leftRms = computeRms(buffer.getChannelData(0));
  const rightRms = computeRms(buffer.getChannelData(1));
  const stronger = Math.max(leftRms, rightRms);
  if (!Number.isFinite(stronger) || stronger < 0.005) {
    return false;
  }

  const weaker = Math.min(leftRms, rightRms);
  return weaker / stronger < ratioThreshold;
};
