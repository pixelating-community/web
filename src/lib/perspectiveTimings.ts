...
export const savePerspectiveTimings = async ({
  ...
  const rows = await sql`
    SELECT p.id, p.audio_src, p.recording_src, p.start_time, p.end_time, p.topic_id, t.locked, t.token, t.name
    FROM perspectives p
  ...
  if (!authToken && requiresWriteToken) {
    throw new TimingsError("MISSING_TOKEN", "Missing token");
  }
  if (requiresWriteToken && authToken) {
  ...
  const isClearAudio = audioSrc === null;
  const isNewRecording =
    hasExplicitAudioSrc && audioSrc !== (row.audio_src ?? null);
  const nextRecordingSrc = isClearAudio
    ? null
    : isNewRecording
      ? audioSrc
      : (row.recording_src ?? null);
  ...
  const storedRecordingSrc =
    isLocked && authToken && isNewRecording && typeof audioSrc === "string" && audioSrc.length > 0
      ? encrypt(audioSrc, authToken)
      : nextRecordingSrc;

  const storedRawRecordingSrc = isNewRecording
    ? storedRecordingSrc
    : null;
  ...
  if (preserveTimings) {
    await sql`
      UPDATE perspectives
      SET recording_src = ${storedRecordingSrc},
          raw_recording_src = COALESCE(${storedRawRecordingSrc}, raw_recording_src)
      WHERE id = ${row.id};
    `;
  } else {
    await sql`
      ...
      SET words_json = ${storedTimings},
          recording_src = ${storedRecordingSrc},
          raw_recording_src = COALESCE(${storedRawRecordingSrc}, raw_recording_src),
  ...
  return {
    timings: preserveTimings ? undefined : (hasAnyTimings ? timings : []),
    audio_src: row.audio_src ?? null,
    recording_src: nextRecordingSrc,
    raw_recording_src: isNewRecording ? nextRecordingSrc : undefined,
    start_time: preserveTimings ? existingStart : nextStart,
    end_time: preserveTimings ? existingEnd : nextEnd,
  };
};
