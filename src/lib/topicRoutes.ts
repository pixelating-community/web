...
export const buildTopicPerspectivePath = ({
  ...
  `${buildTopicPath(topicName)}?r=${encodeURIComponent(perspectiveId.trim())}`;
...
export const buildTopicWritePerspectivePath = ({
  ...
  `${buildTopicPath(topicName)}?w=${encodeURIComponent(perspectiveId.trim())}`;
...
export const buildTopicNewPerspectiveHref = (topicName: string) =>
  `${buildTopicNewPerspectivePath(topicName)}#${encodeURIComponent(NEW_PERSPECTIVE_HASH)}`;
...
export const buildTopicViewerPerspectivePath = ({
  ...
  `${buildTopicPath(topicName)}?p=${encodeURIComponent(perspectiveId.trim())}`;

// Backward-compatible alias retained while removing legacy /w links.
