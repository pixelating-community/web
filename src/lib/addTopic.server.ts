    ...
    if (existing.length > 0) {
      ...
      const updated = await sql.begin(async (tx) => {
        if (existingLocked && !tokenUnchanged) {
          ...
          const rows = await tx`
            SELECT id, perspective, rendered_html, words_json, audio_src, image_src, remix_audio_src
            FROM perspectives
            WHERE topic_id = ${existingTopicId};
          `;
          for (const row of rows as Array<{
            ...
            const nextAudioSrc =
              typeof row.audio_src === "string" && row.audio_src.length
                ...
                : (row.audio_src ?? null);
            const nextImageSrc =
              typeof row.image_src === "string" && row.image_src.length
                ? encrypt(row.image_src, data.token)
                : (row.image_src ?? null);
            const nextRemixAudioSrc =
              typeof row.remix_audio_src === "string" &&
              row.remix_audio_src.length
                ? encrypt(row.remix_audio_src, data.token)
                : (row.remix_audio_src ?? null);
            await tx`
              UPDATE perspectives
              SET perspective = ${nextPerspective},
                  ...
                  remix_audio_src = ${nextRemixAudioSrc}
              WHERE id = ${row.id};
            `;
          }
          ...
          for (const row of rows as Array<{
            ...
            const nextImageSrc =
              typeof row.image_src === "string" && row.image_src.length
                ? decrypt(row.image_src, data.token)
                : (row.image_src ?? null);
            const nextRemixAudioSrc =
              typeof row.remix_audio_src === "string" &&
              row.remix_audio_src.length
                ? decrypt(row.remix_audio_src, data.token)
                : (row.remix_audio_src ?? null);
            await tx`
              UPDATE perspectives
              SET perspective = ${nextPerspective},
