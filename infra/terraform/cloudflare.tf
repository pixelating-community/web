...
resource "cloudflare_ruleset" "root_redirect" {
  count       = var.enable_cloudflare && trimspace(var.preview_subdomain) != "" ? 1 : 0
  zone_id     = var.zone_id
  name        = "Root hostname redirects"
  description = "Redirect apex and www traffic to the preview hostname"
  kind        = "zone"
  phase       = "http_request_dynamic_redirect"

  rules = [
    {
      ref         = "redirect_root_to_preview"
      action      = "redirect"
      description = "Redirect apex and www to preview hostname"
      enabled     = true
      expression  = format("(lower(http.host) eq \"%s\") or (lower(http.host) eq \"www.%s\")", lower(var.domain), lower(var.domain))
      action_parameters = {
        from_value = {
          status_code = 308
          target_url = {
            expression = format("concat(\"https://%s.%s\", http.request.uri.path)", var.preview_subdomain, var.domain)
          }
  ...
  rules = [
    {
      ...
      description = "Cache Vite assets 1 year"
      enabled     = true
    },
    {
      action = "set_cache_settings"
