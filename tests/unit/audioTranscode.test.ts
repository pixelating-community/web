...
describe("transcodeAudioFileToM4a", () => {
  ...
  beforeEach(() => {
    bunStub = {
      file: vi.fn<() => { arrayBuffer: () => Promise<ArrayBuffer> }>(() => ({
        arrayBuffer: vi.fn<() => Promise<ArrayBuffer>>().mockResolvedValue(new Uint8Array([1, 2, 3]).buffer),
      })),
      spawn: vi.fn<() => { exited: Promise<number>; stderr: ReadableStream; stdout: ReadableStream }>(() => ({
        exited: Promise.resolve(0),
        stderr: createReadableStream(),
        stdout: createReadableStream(),
      })),
      write: vi.fn<() => Promise<undefined>>().mockResolvedValue(undefined),
    };
