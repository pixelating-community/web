...
const { encryptMock, sqlMock, verifyTopicTokenMock, buildServerAudioUrlMock } =
  vi.hoisted(() => ({
    encryptMock: vi.fn<(value: string) => string>((value: string) => `enc:${value}`),
...
describe("savePerspectiveTimings", () => {
  ...
  it("persists audio-only recordings without requiring timings", async () => {
    ...
    expect(result).toEqual({
      timings: [],
      audio_src: "old-audio-key",
      recording_src: "new-audio-key",
      raw_recording_src: "new-audio-key",
      start_time: 0,
      end_time: 9.5,
    });
    expect(updates).toHaveLength(1);
    expect(updates[0]).toEqual([null, "new-audio-key", "new-audio-key", 0, 9.5, "p1"]);
  ...
  it("keeps existing bounds when no timings and no duration are provided", async () => {
    ...
    expect(result).toEqual({
      ...
      audio_src: "stored-audio",
      recording_src: null,
      raw_recording_src: undefined,
      start_time: 2.5,
      end_time: 12.25,
    });
    expect(updates).toHaveLength(1);
    expect(updates[0]).toEqual([null, null, null, 2.5, 12.25, "p2"]);
    expect(verifyTopicTokenMock).toHaveBeenCalledWith(
      "writer-token",
      "$argon2id$hash",
    );
